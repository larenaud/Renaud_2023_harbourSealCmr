...
cjs <- nimbleCode({
    ...
    for (i in 1:nind) { # individuals
        ...
        for(t in 1:n.occasions){
            logit(p[i,t]) <-  weaned[i, t] * betaWeaned +
               site_int[i] * betaSite +
              betaYear[year_int[i]] # additive 
    ...
    for(i in 1:16) {
        betaYear[i]~dnorm(mean.p,sd=sd.p)
    }
    # sd.yr~ dunif(0, 5)
...
inits <- function() {
    list(
        ...
        betaYear=rnorm(16, 0, 0.2),
        sd.p=runif(1,.3,1)# 1st reference level is fixed in models, no init
...
chainOut <- nimbleMCMC(
  ...
  #  niter = 1*1000+5000, thin = 1,nburnin = 5000, # 1000 iterations left to estimate parameters
    WAIC=TRUE,
    summary = TRUE,
    samplesAsCodaMCMC = TRUE
)

write_rds(chainOut,file = '~/projects/def-pelleti2/renl2702/phoques/outputs/202305061224_m7b.rds',compress = 'xz')
