...
cjs <- nimbleCode({
    ...
    for (i in 1:nind) { # individuals
        ...
        for(t in 1:n.occasions){
            logit(p[i,t]) <- betaYear[site_int[i]+1,year_int[i]]+ # year effect, as factor
    ...
    sd.bd ~ dunif(1, 20)
    for(y in 1:16) {
        betaYear[1,y]~dlogis(0,1)
        betaYear[2,y]~dlogis(0,1)
        }
...
df$const$Nw = length(df$const$nimbleID)# nimble model
    ...
    list(
      ...
      #  mean.p = runif(1, -1, 0.5),
       ...
       # betaSite = rnorm(1, 0, 1.5),
        betaYear=matrix(rnorm(16*2, -2, .25),nrow = 2,ncol = 16) # 1 er niveau fixe dans les modeles, pas init
...
newOut <- nimbleMCMC(
    ...
    monitors = parameters,
    nchains = 3,
    niter = 50000, thin = 40,nburnin = 10000, # 1000 iterations left to estimate parameters
...
# write_rds(newOut,file=paste0('~/projects/def-pelleti2/renl2702/phoques/outputs/202305082125_m2c.rds'),compress = 'xz')

# change model name in rds object
saveRDS(newOut,file=paste0('~/projects/def-pelleti2/renl2702/phoques/outputs/202305082125_m2c.rds'),compress = 'xz')


goodParam <- c("sd.bd","mu.bd", "weanSurv", "dailySurv","beta","mean")
goodParam <- unlist(sapply(goodParam, function(p) grep(p, x = colnames(newOut$samples[[1]]))))
goodParam <-c(colnames(newOut$samples[[1]])[goodParam],
              paste0("bDate[",sample(1:1000,5),"]")
)
traceplot(newOut$samples[,goodParam])
