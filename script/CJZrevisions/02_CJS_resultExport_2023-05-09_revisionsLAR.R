...
daypath="output.nosync/data/revisions/p"

daypath="output.nosync/data/revisions/phi"
#daypath="cache"
...
# bestOut <- fd[[waic_table$fd.id[4]]]  # lowest WAIC


# plot important parameters
goodParam <- c("sd.bd","mu.bd", "weanSurv", "dailySurv","p.","p.mean","mean.p", "mean.phi", "phi.")
...
# Mean      Median    St.Dev.
# sd.bd        8.5044340   8.4987532 0.25754662
# mu.bd      133.9698746 133.9664871 0.48078344
# weanSurv     0.8167128   0.8173632 0.06628716
# dailySurv    0.9931666   0.9933002 0.00272303
# mean.phi     5.0735696   4.9989483 0.47116315
# mean.p      -2.2823768  -2.2792085 0.14225616
# bDate[901] 129.0206290 129.0334498 3.48852638
# bDate[381] 139.0517535 138.9352974 4.64727496
# bDate[275] 144.0982453 144.1643434 4.54082430
# bDate[861] 119.8588522 119.7679654 3.53630677
# bDate[109] 143.5018992 143.4618337 4.58304997
# 95%CI_low   95%CI_upp
# sd.bd        8.0112901   9.0229066
# mu.bd      133.0322862 134.8927990
# weanSurv     0.6814059   0.9441092
# dailySurv    0.9872948   0.9980847
# mean.phi     4.3529599   6.2559757
# mean.p      -2.5795127  -2.0237886
# bDate[901] 122.1958833 135.9385327
# bDate[381] 129.9495085 148.3556890
# bDate[275] 135.1532729 152.9016854
# bDate[861] 113.1842919 126.8357306
# bDate[109] 134.7608929 152.6141378
...
P.table=betaz %>% transmute(site0=plogis(mean.p),
                            site1=plogis(mean.p+betaSite)) %>% 
    map_dfr(function(x){
        tibble(mean=mean(x),
               cil=quantile(x,0.025),
               cih=quantile(x,0.975)
...
P.table=betaz %>% transmute_at(vars(`betaYear[1]`:`betaYear[16]`),
                               .funs = list(site0=~ plogis(.x ),site1=~ plogis(.x+betaSite ))) %>% 
    map_dfr(function(x){
        tibble(mean=mean(x),
               cil=quantile(x,0.025),
               cih=quantile(x,0.975)
...
P.table=betaz %>% transmute_at(vars(`p.betaYear[1, 1]`:`p.betaYear[2, 16]`),~ plogis(.x)) %>% 
    map_dfr(function(x){
        tibble(mean=mean(x),
               cil=quantile(x,0.025),
               cih=quantile(x,0.975)
...
# 
fig1B=ggplot(Phi.table,aes(x=sex,shape=as.factor(site),y=mean,ymin=cil,ymax=cih))+
    geom_pointrange(position = position_dodge(w=0.25),size=0.8)+
    labs(x='sex',y='Daily survival probability')+
...
fig1A=ggplot(WeanSurv.table,aes(x=year,shape=as.factor(site),y=mean,ymin=cil,ymax=cih))+
    geom_pointrange(position = position_dodge(w=0.25), size=0.8)+
    ...
    cowplot::theme_cowplot(10) +
    theme(panel.grid.major = element_line(color="lightgrey", linetype = 'dotted'), legend.position = 'none')

cowplot::plot_grid(fig1A, fig1B, labels = c('a)', 'b)'), rel_widths = c(0.6, 0.4))
