...
cjs <- nimbleCode({
    ...
    for (i in 1:nind) { # individuals
        ...
        for(t in 1:n.occasions){
            logit(p[i,t]) <- mean.p + 
                ...
                site_int[i] * betaSite + betaYear[year_int[i]] # additive 
    ...
    # interaction
    # betaYearSite[1] <- 0 # reference level for year factor
    # for(i in 2:16) {
    #     betaYearSite[i]~dlogis(0,1)
    # }
...
inits <- function() {
    list(
        ...
        betaYear=c(NA,rnorm(15, 0, 1.5)) # 1 er niveau fixe dans les modeles, pas init
...
saveRDS(newOut,file=paste0('~/projects/def-pelleti2/renl2702/phoques/outputs/202305061433_m2_additive.rds'),compress = 'xz')
