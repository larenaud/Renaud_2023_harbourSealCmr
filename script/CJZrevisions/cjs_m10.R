...
cjs <- nimbleCode({
  ...
  # prior wt-BD model
    ...
    sd.p~dunif(0, 1.5)
for(i in 1:16) {
  phi.betaYear[1,i] <- phi.mean[1]  # remove phi.betaYear terms
  phi.betaYear[2,i] <- phi.mean[2] # remove phi.betaYear terms
}
...
inits <- function() {
    list(
        ...
        p.betaYear=matrix(rnorm(16*2,0,0.5),nrow = 2),
...
chainOut <- nimbleMCMC(
    ...
    monitors = parameters,
    nchains = 3,
    # niter = 5000, thin = 4,nburnin = 1000, # 1000 iterations left to estimate parameters
...
saveRDS(chainOut,file=paste0('~/projects/def-pelleti2/renl2702/phoques/outputs/20230514_2215_m10.rds'),compress = 'xz')
