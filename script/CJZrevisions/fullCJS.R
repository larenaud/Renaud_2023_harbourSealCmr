...
cjs <- nimbleCode({
  ...
  # prior wt-BD model
    ...
    # effet yr*site fixe  pour phi
    
    for(i in 1:16) {
        phi.betaYear[1,i]~dlogis(0, 1)
        phi.betaYear[2,i]~dlogis(0, 1)
        for(t in 1:n.occasions){
            phi.occ[i,t] <- 0
        }
    }
...
inits <- function() {
    list(
       ...
       # p.betaYear=rnorm(16, 0, 0.2),
        p.betaYear = matrix(rnorm(2 * 16, 0, 0.2), nrow = 2),
...
saveRDS(chainOut,file=paste0('~/projects/def-pelleti2/renl2702/phoques/outputs/20230512_1012_intYRSite.rds'),compress = 'xz')
