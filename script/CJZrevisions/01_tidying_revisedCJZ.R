...
str(df.js$data)
nb.fakeID <- 350*16*2
new_yearInt <- rep(1:16,each=350*2)
new_site <-   rep(0:1,350*16)
new_sex <- rep(NA,length(new_site))

df.js$data$y <- rbind(df.js$data$y,matrix(NA,ncol=ncol(df.js$data$y),nrow=nb.fakeID))
...
df.js$const <- list(
    ...
    nimbleID = match(pvData_filtered$myID,row.names(df.js$data$y)),
    first.bd = NA,
