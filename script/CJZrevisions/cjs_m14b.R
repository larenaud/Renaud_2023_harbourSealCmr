...
cjs <- nimbleCode({
  ...
  # prior wt-BD model
    ...
    for(i in 1:16) {
        phi.betaYear[1,i]~dlogis(0, 1.5)
        phi.betaYear[2,i]~dlogis(0, 1.5)
        for(t in 1:n.occasions){
            phi.occ[i,t] <- 0
        }
    }
...
saveRDS(chainOut,file=paste0('~/projects/def-pelleti2/renl2702/phoques/outputs/20230512_1017_m14b.rds'),compress = 'xz')
