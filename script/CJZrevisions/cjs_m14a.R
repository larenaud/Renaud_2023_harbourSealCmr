...
cjs <- nimbleCode({
  ...
  # prior wt-BD model
    ...
    # effet yr+site fix seulement pour phi
    phi.mean[1] ~dlogis(0, 1)
    phi.mean[2] ~dlogis(0, 1)
    for(i in 1:16) {
        bloup[i]~dlogis(0, 1)
        phi.betaYear[1,i] <- phi.mean[1] + bloup[i]
        phi.betaYear[2,i] <- phi.mean[2] + bloup[i]
        for(t in 1:n.occasions){
            phi.occ[i,t] <- 0
        }
    }
...
saveRDS(chainOut,file=paste0('~/projects/def-pelleti2/renl2702/phoques/outputs/20230512_1025_m14a.rds'),compress = 'xz')
