...
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, verbose=FALSE)
...
load(here("output.nosync/data/firstSubmission/2022-12-23_resultsCMR.RData"))
...
# get mean lambda 
```{r}
 t= lbda_bic.3 %>% group_by(yr) %>% filter(yr!=2008&yr!=2002) %>%
...
mean(t$lbda_mean) #1.189304
quantile(t$lbda_mean, 0.025) #0.8371903 
quantile(t$lbda_mean, 0.975) # 1.636451 

tm=lbda_metis.3 %>% group_by(yr) %>% filter(yr!=2000&&yr!=2002&yr!=2013) %>% 
    summarise(lbda_mean=mean(growth),
              lbda_cil=as.numeric(quantile(growth,0.025)),
              lbda_c25=as.numeric(quantile(growth,0.25)),
              lbda_c75=as.numeric(quantile(growth,0.75)),
...
mean(tm$lbda_mean) # 1.090488
quantile(tm$lbda_mean, 0.025) #0.4742506 
quantile(tm$lbda_mean, 0.975) # 1.613211 
...
df_N_na <- data.frame(yr=c(2004:2007, 2017:2018), Nsuper_mean=rep(NA,6), Nsuper_median= rep(NA,6), Nsuper_cil=rep(NA,6), Nsuper_c25=rep(NA,6),Nsuper_c75=rep(NA,6),Nsuper_cih=rep(NA,6), site=rep(c('bic', 'metis'), each=6))
df_N=rbind(df_N,df_N_na)

ggplot(df_N,aes(x=as.factor(yr), shape = as.factor(site)))+
    geom_linerange(aes(ymin=Nsuper_cil,ymax=Nsuper_cih), position=position_dodge(w=0.75))+
   # geom_linerange(aes(ymin=Nsuper_c25,ymax=Nsuper_c75),size=2, position=position_dodge(w=0.75), size=0.75)+
    geom_point(aes(y=Nsuper_mean),size=4, position=position_dodge(w=0.75))+
    labs(x='Year',y='Pup abundance')+
  # theme(panel.grid.minor = element_line(color="transparent"))+
  #scale_colour_manual(labels = c("Bic Island", "Métis"), values = c("bic" = "#FFDB6D", "metis" = "#00AFBB")) + 
...
ggsave("FIGS3_abundanceMedian_revised.png", width=8, height=4.5,unit='in', dpi = 600)
...
ggplot(df_weanSurv,aes(x=as.factor(yr), shape = as.factor(site)))+
  ...
  #  geom_linerange(aes(ymin=weanSurv_c25,ymax=weanSurv_c75),size=2, position=position_dodge(w=0.75), size=0.75)+
    geom_point(aes(y=weanSurv_med), position=position_dodge(w=0.75), size=1)+
