...
for(i in 1:length(dflist)){
  ...
  newOut<-nimbleMCMC(
    ...
    niter = 100000,
    nburnin = 20000,
    thin = 80, #  at 3 = keeps to much mem ??
    nchains = 3, 
    summary = TRUE,
    samplesAsCodaMCMC = TRUE)                                             # Run nimble model
    newOut$samples <- lapply(newOut$samples,function(x) x[,!grepl('z',colnames(x))])
    outlist[[i]] <- newOut
    print('done with:')
    print(i)
    #write_rds(newOut,file=paste0('metis_superpop3_',years[i],'.rds'))
  rm(newOut,data)
}
dur=Sys.time()-start
saveRDS(outlist,file=paste0('~/projects/def-pelleti2/renl2702/phoques/outputs/metis_superpop3.rds'),compress = 'xz')
