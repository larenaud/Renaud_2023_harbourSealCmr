...
for(i in 1:length(dflist)){
  ...
  newOut<-nimbleMCMC(
    ...
    niter = 50000, # 50000
    nburnin = 10000, # 10000
    thin = 40, # doit donner 1000 minimum pour l'extraction 
    nchains = 3)                                             # Run nimble model
    newOut$samples <- lapply(newOut$samples,function(x) x[,!grepl('z',colnames(x))])
    #write_rds(newOut,file=paste0('metis_superpop6_',years[i],'.rds'))
    write_rds(newOut,file=paste0('outputs/metis_superpop6_',years[i],'.rds')) # shitty heavy
  rm(newOut,data)
}
dur=Sys.time()-start
