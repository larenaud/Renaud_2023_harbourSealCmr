...
waic_table <- data.frame(cjs_m = rep(NA,30), WAIC = rep(NA,30), pWAIC=rep(NA,30))
...
for(i in seq_along(file)){
    ...
    waic_table[i,2]<-fd[[i]][["WAIC"]][["WAIC"]]
    waic_table[i,3]<-fd[[i]][["WAIC"]][["pWAIC"]]
}
...
# unzip .rds object 
newOut <- readRDS("202304301954_m6.rds")
newOut <- readRDS("202304301543_m5.rds")

newOut <- readRDS("202305041519_m7b.rds") # pas beau
newOut <- readRDS("202304301743_m7b.rds") # mu bd sd bd pas beau
newOut <- readRDS("202304281441_m7b.rds") # same
newOut <- readRDS("202304261441_m7b.rds") # delete

newOut <- readRDS("202305041522_m2_interactionYrSite.rds") # mu et sd bd pas beau
newOut <- readRDS("202304301536_m2_additive.rds") # no for b weaned b wt

newOut <- readRDS("202304301531_m1.rds")
newOut <- readRDS("20230506_1504_m8.rds")
newOut <- readRDS("202305061158_m4.rds")

# plot important parameters

# all models
plot(newOut$samples[,'mean.phi'], main = "Mean Phi")
plot(newOut$samples[,'weanSurv'], main = "WeanSurv")

plot(newOut$samples[,'mean.p'], main = "mean.p")
plot(newOut$samples[,"sd.p"], main='sd')

plot(newOut$samples[,'betaWeaned'], main = "betaWeaned")
plot(newOut$samples[,'beta.wt'], main = "beta.wt")
plot(newOut$samples[,'mu.bd'], main = "mu.bd")
plot(newOut$samples[,'sd.bd'], main = "sd.bd")
plot(newOut$samples[,'sd.mass'], main = "sd.mass")
plot(newOut$samples[,'dailySurv'], main = "dailySurv")
plot(newOut$samples[,'bDate[3]'], main = "bDate")
plot(newOut$samples[,'bDate[10]'], main = "bDate")
plot(newOut$samples[,'bDate[180]'], main = "bDate")
plot(newOut$samples[,'bDate[50]'], main = "bDate")
plot(newOut$samples[,'bDate[800]'], main = "bDate")

# m2 add
plot(newOut$samples[,'betaYear[1]'], main = "betaYear")
plot(newOut$samples[,'betaYear[4]'], main = "betaYear")
plot(newOut$samples[,'betaYear[8]'], main = "betaYear")
plot(newOut$samples[,'betaYear[12]'], main = "betaYear")
plot(newOut$samples[,'betaYear[16]'], main = "betaYear")

# m4, m5, m7
plot(newOut$samples[,"betaSite"], main='betaSite')
plot(newOut$samples[,"betaYearjj[2, 54]"], main='betaYearjj')
plot(newOut$samples[,"betaYearjj[6, 54]"], main='betaYearjj')
plot(newOut$samples[,"betaYearjj[11, 54]"], main='betaYearjj')

# m2 interactive
plot(newOut$samples[,"betaYearAdd[3]"])
plot(newOut$samples[,"betaYearAdd[16]"])
plot(newOut$samples[,"betaYearAdd[5]"])
plot(newOut$samples[,"betaYearSite[2]"])
plot(newOut$samples[,"betaYearSite[5]"])
plot(newOut$samples[,"betaYearSite[8]"])
plot(newOut$samples[,"betaYearSite[12]"])
plot(newOut$samples[,"betaYearSite[16]"])

plot(newOut$samples[,'betaYear'], main = "site")
newOut$summary$all.chains[grep('betaYear', rownames(newOut$summary$all.chains)),]
...
gelmanCJS.m1 <- gelman.diag(newOut$samples[,'weanSurv'])
gelmanCJS.m1 <- gelman.diag(newOut$samples[,'mean.phi'])


# extract wean surv -------------------------------------------------------
outlist <- readRDS('output.nosync/data/revisions/202304261441_m7b.rds')
...
# extract and plot model parameters ------------------------------------------------------------
...
# plot effect of year on p (if additive)
yearEffect_df<- newOut$summary$all.chains[grep("betaYearAdd", rownames(newOut$summary$all.chains)),] %>% as.data.frame()
# assign year to the df
yearEffect_df$year <- years
colnames(yearEffect_df) <- c('mean', 'median', 'sd', 'ci_low', 'ci_upp', 'year')

ggplot(yearEffect_df,aes(x=as.factor(year), y=mean))+
    geom_point(shape=18,size=4)+
    geom_linerange(data=yearEffect_df,aes(ymin=ci_low,ymax=ci_upp))+
...
# plot effect of site on p (if additive) using the summary - here bic is 0 and metis is 1
siteEffect_df <- newOut$summary$all.chains['betaSite',] %>% t() %>% as.data.frame()

# get the full posterior
siteEffect_df <- data.frame(as.numeric(unlist(newOut$samples[,'betaSite']))) %>% as.data.frame()
