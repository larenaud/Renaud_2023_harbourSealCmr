...
cjs2 <- nimbleCode({
    ...
    for (i in 1:nind) { # individuals
        for (t in f[i]:(n.occasions - 1)) { # occasions
                ...
                sbw*weaned[i,t] +
                ranef.yr[OccuN[t]] # this is the daily survival variation
        }
...
newOut <- nimbleMCMC(
    ...
    # niter = 10000, thin = 5,nburnin = 5000,
    WAIC=TRUE,
    summary = TRUE,
...
saveRDS(outlist,file=paste0('~/projects/def-pelleti2/renl2702/phoques/outputs/cjs3_SemiRanef_metis.rds'),compress = 'xz')
