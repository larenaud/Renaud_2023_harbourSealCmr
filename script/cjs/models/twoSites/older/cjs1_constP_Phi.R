...
# code adapted from Kéry and Schaub 2012. This is to run model on both sites combined. Was modified 2022 08 21 to reformat pup_ch and pupData_list per year. 
...
for(i in 1:16){ # treat each year separately
    ...
    captureJJ = unique(pupData_list[[now]]$julianDay) # changed bicData35_l to pupData_list
    allJJ <- seq(min(captureJJ),max(captureJJ))
    ch_dum <- matrix(0,nrow = nrow(pup_ch[[now]]),ncol = length(allJJ)) # list_ch_bic to pup_ch
...
saveRDS(outlist,file=paste0('~/projects/def-pelleti2/renl2702/phoques/outputs/cjs1_constP_Phi_combined.rds'),compress = 'xz')
