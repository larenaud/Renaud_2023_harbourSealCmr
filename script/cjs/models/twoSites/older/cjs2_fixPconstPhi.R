...
# model with temporal variation in capture probability (fixed time effects) but constant survival across time and identical for all individuals
...
# mydat for all yrs -------------------------------------------------------
load("~/projects/def-pelleti2/renl2702/phoques/2022-08-22_pupDataCjsCombined.RData")
...
cjs2 <- nimbleCode({
    ...
    for (i in 1:nind) { # individuals
        for (t in f[i]:(n.occasions - 1)) { # occasions
                logit(phi[i,t]) <- mean.phi+ #constant survival
                sbw*weaned[i,t] # +
                # ranef.yr[t] # this is the daily survival variation
        }

        for(t in 1:nrealOcc){
            logit(p[i, realOcc[t]]) <- beta[t] + # fixed effect of time on p
    ...
    for(t in 1:(n.occasions-1)){
      ...
      beta[t]~dlogis(0,1) # priors for time-spec. recapture or should it be dunif?
    }
...
for(i in 1:16){ # treat each year separately
    ...
    #mydata <- pup_ch[[now]]
    captureJJ = unique(pupData_list[[now]]$julianDay)
    allJJ <- seq(min(captureJJ),max(captureJJ))
    ch_dum <- matrix(0,nrow = nrow(pup_ch[[now]]),ncol = length(allJJ))
    tmp <- match(captureJJ, allJJ)
    for(i2 in 1:length(tmp)){
        ch_dum[,tmp[i2]] <- pup_ch[[now]][,i2]
    }
    rownames(ch_dum) <- rownames(pup_ch[[now]])
    get.first <- function(x)    min(which(x != 0))

    dflist[[i]]<-list(
        data=list(y=as.matrix(ch_dum),
                  mass=pupData_list[[now]]$mass),
        const=list()
    )
    dflist[[i]]$const <- list(f=apply(dflist[[i]]$data$y, 1,function(x) get.first(x)),
                              ...
                              Nw =  nrow(pupData_list[[now]]),
                              weanedAge = 30,# give a value
                              julianDay=pupData_list[[now]]$julianDay,
                              # mass = pupData_list[["2009"]]$mass,
                              nimbleID = pupData_list[[now]]$nimbleID,
                              first.bd=NA,
    ...
    tmptmp=  pupData_list[[i]] %>% group_by(myID) %>% summarise(min.bd = min(julianDay)) # pull vector of minimal dates for existing ID - their entry date
    tmptmp=  tmptmp$min.bd[match(rownames(dflist[[i]]$data$y),tmptmp$myID)]
    dflist[[i]]$const$first.bd <- ifelse(is.na(tmptmp),max(unique(pupData_list[[i]]$julianDay)),tmptmp)
...
saveRDS(outlist,file=paste0('~/projects/def-pelleti2/renl2702/phoques/outputs/cjs2_fixPconstPhi_combined.rds'),compress = 'xz')
