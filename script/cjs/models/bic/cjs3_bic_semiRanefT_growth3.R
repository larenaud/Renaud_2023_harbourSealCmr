...
#load("/Users/LimoilouARenaud/Documents/PostDocI/Projects/cmrSeal/data/mine/old/20211031_cmr_pup35.RData")
...
cjs2 <- nimbleCode({
    ...
    for (j in 1:Nw) {
        # nb lines mass 1998
        wt.hat[j] <- 8 + beta.wt * (julianDay[j] - round(bDate[nimbleID[j]])) # Dubé 2003 - 10 kg at birth # changed to 8 since 10 occurred before first bd
    ...
    for (i in 1:nind) { # individuals
        for (t in f[i]:(n.occasions - 1)) {
                logit(phi[i,t]) <- mean.phi[(weaned[i, t]+1)] +
                   # sbw*weaned[i,t] +
                    ranef.t[OccuN[t]] # added OccuN before [t] for daily surv variation
        }

        for(t in 1:nrealOcc){
            logit(p[i, realOcc[t]]) <- beta[t, (weaned[i, t]+1)]
        }
 ...
 # priors for time-spec. recapture # added here with addition of OccuN instead of l. 55
    ...
    betaWeaned <- -4 #~ dnorm(0, 0.001)
    mean.phi[1] ~dlogis(0, 1)# Prior for mean survival
    mean.phi[2] ~dlogis(0, 1)# Prior for mean survival
  ...
  #  sd.ip~ dunif(0, 5)
    ...
    logit(dailySurv) <- mean.phi[1] # so daily surv takes the average surv not the random time variation
...
inits <- function() {
    list(
        mean.phi = rnorm(1, 4, 1),
        beta = matrix(runif(2*dflist[[i]]$const$nrealOcc, -1, 0.5),ncol=2,nrow = dflist[[i]]$const$nrealOcc),
...
newOut <- nimbleMCMC(
    ...
    #niter = 25000, thin = 5,nburnin = 20000,
    WAIC=TRUE,
    summary = TRUE,
...
saveRDS(outlist,file=paste0('~/projects/def-pelleti2/renl2702/phoques/outputs/cjs3_bic_semiRanefTGrowth3-7dec20h.rds'),compress = 'xz')
