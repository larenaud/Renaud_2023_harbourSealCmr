...
cjs2 <- nimbleCode({
    ...
    for (i in 1:nind) { # individuals
        ...
        for(t in 1:nrealOcc){
            logit(p[1:nind, realOcc[t]]) <- beta[t]
  ...
  #  sd.ip~ dunif(0, 5)
    ...
    for (i in 1:nind) {
        ...
        z[i, f[i]] <- 1
        lastOc[i] <- min(max((round(bDate[i])+weanedAge-firstOcc+1),(f[i] + 1)),n.occasions)
        # for (t in (f[i] + 1):n.occasions) {
        for (t in (f[i] + 1):n.occasions){   # potential alternative to really truncate after weaning, but beta.weaned and sbw will not longuer be identifiaable
...
dflist<-list()
outlist <- list()
for(i in 1){ # treat each year separately
...
# saveRDS(outlist,file=paste0('~/projects/def-pelleti2/renl2702/phoques/outputs/cjs3_bic_semiRanefYr.rds'),compress = 'xz')

saveRDS(outlist,file=paste0('outputs/cjs3_bic_semiRanefYrT.rds'),compress = 'xz')
