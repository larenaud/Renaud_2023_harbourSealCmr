...
# model with random temporal variation in survival, and individual variation in p but no time effect on p
...
cjs2 <- nimbleCode({
    ...
    for (i in 1:nind) { # individuals
        ...
        for(t in 1:nrealOcc){
            logit(p[i, realOcc[t]]) <- mean.p + weaned[i, t] * betaWeaned + ranef.id[i]
        }
    ...
    # for(t in 1:nrealOcc){
    #     beta[t]~dlogis(0,1) # no fixed time effect on p in this model
    # }
    # 
    for(t in 1:(nrealOcc-1)){
        ranef.t[t]~ dnorm(0, sd=sd.t) # priors for time-spec  # OccuN instead of l. 55
    }
    ...
    sbw ~ dnorm(0, 0.001)
    sd.t~ dunif(0, 5) # changed to sd.t instead of sd.yr
    sd.ip~ dunif(0, 5)
...
saveRDS(outlist,file=paste0('~/projects/def-pelleti2/renl2702/phoques/outputs/cjs6_metis_semiRanefTID_Growth2.rds'),compress = 'xz')
