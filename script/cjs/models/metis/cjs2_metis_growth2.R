...
cjs2 <- nimbleCode({
    ...
    for (i in 1:nind) { # individuals
        ...
        for(t in 1:nrealOcc){
            logit(p[i, realOcc[t]]) <- beta[t]+ weaned[i, t] * betaWeaned
        }
...
#    }
  ...
  #  sd.ip~ dunif(0, 5)
    ...
    sd.mass ~ dunif(0, 10)
    sd.bd ~ dunif(1, 20) # changed from 0 to 1
...
saveRDS(outlist,file=paste0('~/projects/def-pelleti2/renl2702/phoques/outputs/metis_cjs2_growth2.rds'),compress = 'xz')
