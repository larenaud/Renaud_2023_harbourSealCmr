...
cjs2 <- nimbleCode({
    ...
    for (i in 1:nind) { # individuals
        ...
        for(t in 1:nrealOcc){
            logit(p[i, realOcc[t]]) <- mean.p + weaned[i, t] * betaWeaned # no ran id, no beta[i]
        }
...
saveRDS(outlist,file=paste0('~/projects/def-pelleti2/renl2702/phoques/outputs/metis_cjs1_Growth2.rds'),compress = 'xz')
