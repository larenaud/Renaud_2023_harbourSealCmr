...
for(i in 1:length(years)){ # treat each year separately
    ...
    get.first <- function(x)
        min(which(x != 0))
    f <- apply(list_ch_bic[[now]], 1, get.first)
    
    dflist[[i]]<-list(
        data=list(y=as.matrix(list_ch_bic[[now]]),
...
saveRDS(newOut,file=paste0('~/projects/def-pelleti2/renl2702/phoques/outputs/bic_cjs_',years[i],'.rds'))
rm(newOut,data)
